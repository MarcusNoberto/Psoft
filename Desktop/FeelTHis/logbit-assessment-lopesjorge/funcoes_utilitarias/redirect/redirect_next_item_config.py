...
def redirect_next_item_config(video_pergunta: object) -> HttpResponseRedirect:
    ...
    if curso.ultimo_item == video_pergunta:
        return reverse_lazy_plus(
            ...
            url_params=[
                video_pergunta.modulo.id,
                video_pergunta.modulo.curso.projeto.slug
            ]
...
def get_next_itens(video_pergunta, itens):
    ...
    return list(
        filter(
            ...
            item.ordem > video_pergunta.ordem, # Condições que eu quero
            itens # Lista
        )
    )
...
def tem_check(video_pergunta):
    ...
    if video_pergunta.tipo == 'video':
        ...
        return not video_pergunta.resposta_set.all().exists()
    else:
        return False
...
def redirect_pergunta(redirect_function, pergunta):
    return redirect_function(
        url_params=[
            pergunta.tipo,
            pergunta.id,
            pergunta.modulo.curso.projeto.slug
        ]
...
def redirect_video(redirect_function, video):
    return redirect_function(
        url_params=[
            video.modulo.id,
            video.id,
            video.modulo.curso.projeto.slug
        ]
    )
